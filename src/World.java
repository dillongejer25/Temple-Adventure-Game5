...
public class World {
        ...
        try {
            ...
            for (int y = 0; y < rows.size(); y++) {
                ...
                for (int x = 0; x < cols.length; x++) {           // Counter control loop
                    tile_name = cols[x];
                    if (tile_name.equals("EnterTemple")) {
    ...
    public static MapTile tile_exists(int x, int y) {
        ...
        if ((x >= 0 && x < _world.length) && (y >= 0 && y < _world[0].length)
            ...
            switch (_world[x][y]) {
                case "EnterTemple":
                    mt = new EnterTemple(x, y);
                    mt = checkRoomExists(mt);
                    break;
                case "SuperSwordRoom":

                    mt = new SuperSwordRoom(x, y,new SuperSword());
                    mt = checkRoomExists(mt);
                    break;
                case "GiantSnakeRoom":
                    mt = new GiantSnakeRoom(x, y,new GiantSnake());
                    mt = checkRoomExists(mt);
                    break;
                case "SandRoom":
                    mt = new SandRoom(x, y, new Gems(15));
                    mt = checkRoomExists(mt);
                    break;
                case "DarkRoom":
                    mt = new DarkRoom(x, y);
                    break;
                case "FindLazerRoom":
                    mt = new FindLazerRoom(x,y,new Lazer());
                    mt = checkRoomExists(mt);
                    break;
                case "GuardRoom":
                    mt = new GuardRoom(x, y, new Guard());
                    mt = checkRoomExists(mt);
                    break;
                case "ExitTemple":
                    mt = new ExitTemple(x, y);
                    break;
